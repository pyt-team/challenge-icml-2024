...
def plot_manual_hypergraph(data, scale_factor=1.1):
    ...
    if hasattr(data, "incidence_hyperedges"):
        ...
        hyperedges = [
            list(
                torch.nonzero(incidence_matrix[:, i], as_tuple=False)
                .squeeze()
                .numpy()
            )
        ...
        raise ValueError(
            "Data object does not have 'incidence_hyperedges' attribute"
        )
    ...
    nx.draw_networkx_edges(
        G, pos, edgelist=G.edges(), edge_color="grey", width=1.5
    )
    ...
    for hyperedge in hyperedges:
        ...
        if (
            ...
            scaled_points = centroid + scale_factor * (
