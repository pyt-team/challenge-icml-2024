...
class TestDirectedSimplicialCliqueLifting:
    ...
    def test_lift_topology(self):
        ...
        expected_triangle_incidence_2 = torch.tensor([
            [1.],
            [1.],
            [1.]
        ])
        ...
        three_two_lifted_data = self.three_two_lifting.forward(self.three_two_data.clone())
        ...
        assert (
            expected_three_two_incidence_1 == three_two_lifted_data.incidence_1.to_dense()
        ...
        assert (
            expected_three_two_incidence_2 == three_two_lifted_data.incidence_2.to_dense()
        ).all(), "Something is wrong with three_two incidence_2 (edges to triangles)."
        
        # Test missing_triangle lifting
        missing_triangle_lifted_data = self.missing_triangle_lifting.forward(self.missing_triangle_data.clone())
        ...
        assert (
            expected_missing_triangle_incidence_1 == missing_triangle_lifted_data.incidence_1.to_dense()
        ).all(), "Something is wrong with missing_triangle incidence_1 (nodes to edges)."
        ...
        assert (
            expected_missing_triangle_incidence_2 == missing_triangle_lifted_data.incidence_2.to_dense()
        ).all(), "Something is wrong with missing_triangle incidence_2 (edges to triangles)."
